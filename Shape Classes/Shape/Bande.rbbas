...
Inherits Shape
	#tag Method, Flags = &h0
		Sub Bande(ol as objectslist, p as BasicPoint)
	...
	#tag Method, Flags = &h0
		Sub Fixecoord(p as BasicPoint, n as integer)
		  ...
		  case 1
		    Points(1).moveto(p)
		    figskull(sk).Updatesommet(1,wnd.myCanvas1.dtransform(p-Points(0).bpt))
		    Points(2).moveto(p)
		  case 2
		    Points(2).moveto(p)
		    constructshape
	...
	#tag Method, Flags = &h0
		Sub updateskull()
		  ...
		  for i=0 to 3
		    if i=0 then
		      sk.update(wnd.myCanvas1.transform(extre(0)))
		    else
		      figskull(sk).Updatesommet(i,wnd.myCanvas1.dtransform(extre(i)-extre(0)))
		    end
		  next
	...
	#tag Method, Flags = &h0
		Sub Bande(obl as objectslist, s as bande, p as basicpoint)
		  dim M as Matrix
		  ...
		  sk = new Figskull(wnd.mycanvas1.transform(s.Points(0).bpt),4)
		  computeextre
		  Move(M)
	...
	#tag Method, Flags = &h0
		Sub ToEPS(tos as TextOutputstream)
		  ...
		  if not hidden then
		    p3 = Point3
		    tos.writeline ( "[ " + Points(0).etiq +" "+ Points(1).etiq+ " ]  droite" )
		    tos.writeline ( "[ " + Points(2).etiq+" [ " + str(p3.x) + " " + str(p3.y) + " ]  ]  droite" )
		  end if
	...
	#tag Method, Flags = &h0
		Sub Bande(B as Bande, M as Matrix)
		  Shape(B, M)
		End Sub
	#tag EndMethod
