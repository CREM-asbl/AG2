...
Inherits Quadri
	...
	#tag Method, Flags = &h0
		Sub Trap(ol as objectslist, p as basicPoint)
		  Quadri(ol,4,p)
		  liberte = 7
	...
	#tag Method, Flags = &h0
		Function Stability(Byref Pt as Point, Q as point) As Boolean
		  ...
		  if dist < wnd.Mycanvas1.MagneticDist*2 then
		    if Stab1 = nil then
		      Stab1 = new Basicpoint(Pt)
		      Stab2 = new BasicPoint(Q.bpt*2-Pt.bpt)
		    end if
	...
	#tag Method, Flags = &h0
		Function trapupdate1(p as point) As Boolean
		  ...
		  p1 = point(ff.somm.element((nn+2)mod 4))
		  n1 = getindexpoint(p1)
		  p2 = point(ff.somm.element((nn+3)mod 4))
		  n2 = getindexpoint(p2)
	...
	#tag Method, Flags = &h0
		Function trapupdate2(p1 as point, p2 as point) As Boolean
		  ...
		  n1 = getindexpoint(p1)
		  n2 = getindexpoint(p2)
		  p = point(ff.somm.element(ff.fx1))
		  n = getindexpoint(p)
		  // p est le point fixe
