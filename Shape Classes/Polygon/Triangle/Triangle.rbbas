...
Inherits Polygon
	#tag Method, Flags = &h0
		Sub Triangle(ol as objectslist, d as integer, p as basicpoint)
		  dim i as integer
		  Polygon(ol,d,p)
		  Npts = 3
		  redim prol(2)
	...
	#tag Method, Flags = &h0
		Sub Triangle(ol as objectslist, fp as point, sp as point, tp as point)
		  shape(ol)
		  fam = 2
		  forme = 0
		  npts = 1
	...
	#tag Method, Flags = &h0
		Function TroisiemeSommet(n1 as integer, n2 as integer) As integer
		  dim i, n as integer
	...
	#tag Method, Flags = &h0
		Sub Triangle(ol as ObjectsList)
		  Shape(ol,3,3)
		  redim prol(2)
