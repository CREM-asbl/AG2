...
Inherits Trap
	...
	#tag Method, Flags = &h0
		Function Paste(Obl as Objectslist, p as Basicpoint) As TrapRect
		  return new TrapRect(Obl,self,p)
	...
	#tag Method, Flags = &h0
		Function trapupdate1(pp as point) As boolean
		  ...
		  case 0, 1
		    m = getindexpoint(point(ff.somm.item(ff.PointsFixes(0))))
		    if m = (n+2) mod 4 then
		      if n = 0 or n = 2 then
		  ...
		  case 2
		    m1 = getindexpoint(point(ff.somm.item(ff.PointsFixes(0))))
		    m2 = getindexpoint(point(ff.somm.item(ff.PointsFixes(1))))
