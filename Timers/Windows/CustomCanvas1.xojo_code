...
Inherits Canvas
		...
		Function ConstructContextualMenu(base as MenuItem, x as Integer, y as Integer) As Boolean
		  ...
		  if icot = -1 then
		    ...
		    if (not sctxt isa point) and (not sctxt isa droite) and (not sctxt isa arc) then
		      ...
		      dim SubMenu as New MenuItem(Dico.Value("TransparencyChange"))
		      submenu.append New MenuItem(Dico.Value("ToolsOpq"))
		      submenu.append New MenuItem(Dico.Value("ToolsSTsp"))
		      submenu.append New MenuItem(Dico.Value("ToolsTsp"))
		      base.append submenu
		    end if
	...
	#tag Method, Flags = &h0
		Sub drawzone1(bp as BasicPoint)
		  if currentcontent.currentoperation <> nil then
		    background.graphics.drawobject zone, bp.x, bp.y
		  end if
		End Sub
	...
	#tag Method, Flags = &h0
		Sub PreparerChoixContext(p as BasicPoint)
