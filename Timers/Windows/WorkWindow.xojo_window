...
#tag WindowCode
	...
	#tag MenuHandler
		Function ToolsOpq() As Boolean Handles ToolsOpq.Action
			...
			TransparencyChange(CurrentContent.CurrentOperation).ImmediateDoOperation(100)
			refreshtitle
			end if
			return true
		End Function
	...
	#tag MenuHandler
		Function ToolsSTsp() As Boolean Handles ToolsSTsp.Action
			...
			TransparencyChange(CurrentContent.CurrentOperation).ImmediateDoOperation(50)
			refreshtitle
			can.invalidate
			end if
			return true
	...
	#tag MenuHandler
		Function ToolsTsp() As Boolean Handles ToolsTsp.Action
			...
			TransparencyChange(CurrentContent.CurrentOperation).ImmediateDoOperation(0)
			refreshtitle
			end if
			return true
	...
	#tag MenuHandler
		Function TransfosTrajectoire() As Boolean Handles TransfosTrajectoire.Action
			if mousedispo then
			closefw
			CurrentContent.CurrentOperation=new TrajectoireTsf()
			refreshtitle
			end if
			return true
	...
	#tag Method, Flags = &h0
		Sub colorchange(b as boolean)
		  ...
		  if mousedispo then
		    closefw
		    call selectcolor(col,"Choisis une couleur")
		...
		Sub deleteContent()
		  ...
		  if not currentcontent.macrocreation then
		    if CurrentContent.TheObjects.count > 1 and not CurrentContent.CurrentFileUptoDate then
		      conf = new Confirmation
		      conf.Label1.Text = Dico.Value("savepic")+ " " + str(CurrentContent.id)+" ?"
		      Conf.ShowModal
		      val = Conf.result      ''Yes
		      conf.close
		    end if
	...
	#tag Method, Flags = &h0
		Sub SetIco(Fam as integer, Form As integer)
		  dim h, taille, haut, larg as double
		  dim specs as StdPolygonSpecifications
		  dim Dimen as BasicPoint
		  ...
		  elseif ubound(specs.angles) > 0 then
		    ...
		    dimen = ico(Fam).BB.Second-ico(fam).BB.First
		    larg = dimen.x
		    haut = dimen.y
		    taille = 0.8*h/max(larg,haut)
		    ico(Fam).Update new BasicPoint (3-ico(fam).BB.First.x*taille,h-5-ico(fam).BB.Second.y*taille)
		  else
		    'ico(fam) = new Ovalskull(h/2,new BasicPoint(h/2,h/2))
	...
	#tag Method, Flags = &h0
		Sub transparchange(f as integer)
		  'refreshtitle
		  'if mousedispo then
		  'closefw
	...
	#tag Event
		Sub Paint(index as Integer, g As Graphics, areas() As REALbasic.Rect)
		  if index < Config.nstdfam then
		    ...
		    g.drawobject self.ico(index) 'me.paint(g)
