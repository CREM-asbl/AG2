...
Inherits SelectOperation
	...
	#tag Method, Flags = &h0
		Sub adapterparametiq(lab as label, tos as textoutputstream)
		  dim font as string
	...
	#tag Method, Flags = &h0
		Sub adjoindrepoint(tos as TextOutputstream, q as point)
		  ...
		  if (not q.hidden or (ubound(q.parents) > -1 ) )  and not q.invalid and not q.deleted then
		    ...
		    if Obeps(n) = 0 then
		      Obeps(n) = 1
		      tos.writeline("/"+q.etiq+ " [" + str(q.bpt.x) + " " + str(q.bpt.y) +"]   store")
		    end if
	...
	#tag Method, Flags = &h0
		Sub SaveEPS(s as string, name as string, tip as boolean)
		  SaveEPS
		  disp = s
		  ti = tip
		  nom = name
